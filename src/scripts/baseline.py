...
THIS ONLY NEEDS TO BE RUN WHEN BASELINES NEED TO BE ESTABLISHED, DOES NOT NEED TO BE RUN FOR
REGULAR REPORTING
"""
...
#To remove dependence on ACE gateway, importing data from our folder

# Defining the main directory
main_dir = r"F:\PROJECTS\1715 Main Street Landing EMIS Pilot\Monthly Reports"

# Defining regular expression (regex) pattern to match filenames like "Report_df_final_YYYY-MM-DD.csv"
pattern = re.compile(r"Report_df_final_\d{4}-\d{2}-\d{2}\.csv")
...
Baseline_df['Total DD'] = Baseline_df['HDD'] + Baseline_df['CDD']

columns_to_check = ['Total Heating Plant Energy Consumption (MMBtu)', 'AHU 19 Total kW (Correlated)',
                    'HRU Total kW (Correlated)', 'Total CHW kW', 'HDD', 'CDD']

# # Drop rows where all the specified columns have NaN values
Baseline_df= Baseline_df.dropna(subset=columns_to_check)
...
# Grouping by day
# Converting it to daily data because that improves model r squared
# Only proceed if 'Unnamed: 0' column exists
...
# Baseline_df.dropna(subset=['DateTime'], inplace=True)
...
plt.scatter(Baseline_df['temperature_2m'], Baseline_df['Total Boiler NG Consumption (MBtu)'])
...
plt.figure(figsize=(10, 6))
plt.scatter(Baseline_df['temperature_2m'], Baseline_df['Heating System kW'])
plt.xlabel('Average Temperature (F)')
...
plt.figure(figsize=(10, 6))
plt.scatter(Baseline_df['temperature_2m'], Baseline_df['AHU 19 Total kW (Correlated)'])
plt.xlabel('Average Temperature (F)')
...
plt.figure(figsize=(10, 6))
plt.scatter(Baseline_df['temperature_2m'], Baseline_df['HRU Total kW (Correlated)'])
plt.xlabel('Average Temperature (F)')
plt.ylabel('HRU kW Usage')
plt.savefig(r'F:\PROJECTS\1715 Main Street Landing EMIS Pilot\code\Plots\Baseline\HRU_temp_kW.png')
plt.close()

#Plotting Ventilation System kW consumption vs temp
plt.figure(figsize=(10, 6))
plt.scatter(Baseline_df['temperature_2m'], Baseline_df['Ventilation_kW'])
plt.xlabel('Average Temperature (F)')
plt.ylabel('Ventilation kW Usage')
plt.savefig(r'F:\PROJECTS\1715 Main Street Landing EMIS Pilot\code\Plots\Baseline\Ventilation_temp_kW.png')
plt.close()
...
Heating_model = LinearRegression().fit(
    Baseline_df['HDD'].values.reshape(-1, 1),
    Baseline_df['Total Heating Plant Energy Consumption (MMBtu)']
)

AHU19_model = LinearRegression().fit(
    Baseline_df['Total DD'].values.reshape(-1, 1),
    Baseline_df['AHU 19 Total kW (Correlated)']
)

HRU_model = LinearRegression().fit(
    Baseline_df['temperature_2m'].values.reshape(-1, 1),
    Baseline_df['HRU Total kW (Correlated)']
)

Ventilation_model_DD = LinearRegression().fit(
    Baseline_df['Total DD'].values.reshape(-1, 1),
    Baseline_df['Ventilation_kW']
)

Ventilation_model_temp = LinearRegression().fit(
    Baseline_df['temperature_2m'].values.reshape(-1, 1),
    Baseline_df['Ventilation_kW']
)
...
with open(r'F:\PROJECTS\1715 Main Street Landing EMIS Pilot\code\Baseline_Model_Regression_Parameters.csv', 'w',
    ...
    writer.writerow([
        'Ventilation_model_DD',
        float(Ventilation_model_DD.coef_),
        float(Ventilation_model_DD.intercept_),
        Ventilation_model_DD.score(
            Baseline_df['Total DD'].values.reshape(-1, 1),
            Baseline_df['Ventilation_kW']
        )
    ])
    writer.writerow([
        'Ventilation_model_temp',
        float(Ventilation_model_temp.coef_),
        float(Ventilation_model_temp.intercept_),
        Ventilation_model_temp.score(
            Baseline_df['temperature_2m'].values.reshape(-1, 1),
            Baseline_df['Ventilation_kW']
        )
    ])
...
#Heating Model
x = Baseline_df['HDD'].values.reshape(-1, 1)
y = Baseline_df['Total Heating Plant Energy Consumption (MMBtu)'].values
...
#AHU19 Model
x = Baseline_df['Total DD'].values.reshape(-1, 1)
y = Baseline_df['AHU 19 Total kW (Correlated)'].values

# Predicting y values using the fitted model
y_pred = AHU19_model.predict(x)
...
# Adding labels and title
plt.xlabel('Total DD')
plt.ylabel('AHU 19 Total kW')
plt.title('Linear Regression: Degree Days vs. AHU 19 Total kW')

# Adding a legend
plt.legend()

# Showing the plot
plt.savefig(r'F:\PROJECTS\1715 Main Street Landing EMIS Pilot\code\Plots\AHU19Model_with regression line.png')
plt.close()

#HRU Model
x = Baseline_df['temperature_2m'].values.reshape(-1, 1)
y = Baseline_df['HRU Total kW (Correlated)'].values

# Predicting y values using the fitted model
y_pred = HRU_model.predict(x)
...
plt.ylabel('HRU Total kW')
plt.title('Linear Regression: Temperature vs. HRU Total kW')

# Adding a legend
plt.legend()

# Showing the plot
plt.savefig(r'F:\PROJECTS\1715 Main Street Landing EMIS Pilot\code\Plots\HRUModel_with regression line.png')
plt.close()

#Ventilation_model_temp
x = Baseline_df['temperature_2m'].values.reshape(-1, 1)
y = Baseline_df['Ventilation_kW'].values

# Predicting y values using the fitted model
y_pred = Ventilation_model_temp.predict(x)
...
# Adding labels and title
plt.xlabel('Temperature (F)')
plt.ylabel('Ventilation Total kW')
plt.title('Linear Regression: Temperature vs. Ventilation Total kW')

# Adding a legend
plt.legend()

# Showing the plot
plt.savefig(r'F:\PROJECTS\1715 Main Street Landing EMIS Pilot\code\Plots\Ventilation_model_temp_with regression line.png')
plt.close()
...
# Predicting y values using the fitted model
y_pred = Ventilation_model_DD.predict(x)
...
plt.xlabel('Total DD')
plt.ylabel('Ventilation Total kW')
plt.title('Linear Regression: Degree Days vs. Ventilation Total kW')

# Adding a legend
plt.legend()

# Showing the plot
plt.savefig(r'F:\PROJECTS\1715 Main Street Landing EMIS Pilot\code\Plots\Ventilation_model_DD_with regression line.png')
plt.close()